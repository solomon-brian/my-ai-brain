...
export default function Home() {
    ...
    const addNote = () => { if (!input.trim()) return; setNotes([...notes, { text: input, date: new Date().toISOString() }]); setInput(""); };
    const clearNotes = () => { if (window.confirm("Are you sure?")) { setNotes([]); } };
    ...
    const handleKeyPress = (e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); mode === 'note' ? addNote() : askAI(); } };
                ...
                <div className="w-full max-w-2xl mx-auto h-full flex flex-col">
                    ...
                    <div className="flex-1 min-h-0 relative">
                        ...
                        <div className={`absolute inset-0 transition-opacity duration-300 ${mode === 'note' ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                            ...
                            <div className="h-[calc(100%-100px)] overflow-y-auto space-y-2 pr-2">
                                {notes.length > 0 ? notes.slice().reverse().map((n, i) => (
                                    <div key={i} className="p-3 rounded-lg text-sm" style={{backgroundColor: 'var(--surface-bg)', color: 'var(--text-dim)'}}>{n.text}</div>
                                )) : (
                        ...
                        <div className={`absolute inset-0 transition-opacity duration-300 ${mode === 'ask' ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                            <h2 className="text-lg font-bold mb-2 px-1" style={{color: 'var(--text-main)'}}>AI Response</h2>
